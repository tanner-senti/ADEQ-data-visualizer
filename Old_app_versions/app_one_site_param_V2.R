...
# 1. ALways pulls from the LIMS SQL server each time the app is opened (slow, but always recent data)
# 2. Only allows one site/parameter selection each time
# 3. Instantly shows data (fast and cool to see)
...
server <- function(input, output, session) {
  ...
  fetch_data <- reactive({
    ...
    tryCatch(
      {
        ...
        runjs(
          'document.getElementById("loading-message").textContent = "Initializing database...";'
        )

        # Create sqlite database for data
        full_con <- dbConnect(
          duckdb::duckdb(),
          paste0(temp_dir, "/weblims_full.duckdb")
        )
    ...
    if (!server_grab) {
      runjs(
        'document.getElementById("loading-message").textContent = "Using backup database...";'
      )
      ...
      db_message <- paste(
        "Using backup database - data available between ",
        date_range$min_date,
        " and ",
        date_range$max_date
      )
      ...
      dbExecute(
        full_con,
        "ALTER TABLE WebLIMSResults RENAME COLUMN StationID to SamplingPoint"
      )
  ...
  observe({
    ...
    suppressWarnings(updateSelectizeInput(
      session,
      "site",
      choices = site_info$SamplingPoint,
      server = FALSE
    )) # Set to FALSE during debugging
    suppressWarnings(updateSelectizeInput(
      session,
      "description",
      choices = site_info$Description,
      server = FALSE
    ))
  })
  ...
  observeEvent(input$site, {
    ...
    if (!is.null(con)) {
      query <- paste(
        "SELECT DISTINCT WebParameter FROM WebLIMSResults WHERE SamplingPoint = '",
        input$site,
        "'",
        sep = ""
      )
      ...
      updateSelectizeInput(
        session,
        "parameter",
        choices = parameters$WebParameter,
        server = TRUE
      )
    }
  })
  ...
  filtered_data <- reactive({
    ...
    if (!is.null(input$date_range)) {
      df <- df %>%
        filter(
          DateSampled >= input$date_range[1],
  ...
  make_plot <- function(selected_plot_dat) {
    ...
    p <- ggplot(
      ...
      geom_point_interactive(
        aes(color = Qualifiers, shape = DL),
        alpha = 0.7,
        size = 2.5
      ) +
      scale_shape_manual(
        values = c("Measured value" = 16, "<DL" = 17, ">DL" = 17)
    ...
    if (use_size) {
      p <- p +
        geom_point_interactive(
          aes(size = RelativeDepthComments, color = Qualifiers, shape = DL),
          alpha = 0.7
        ) +
  ...
  output$data_table <- DT::renderDT(
    {
      ...
      clean_data$DateSampled <- format(
        as.Date(clean_data$DateSampled),
        "%Y-%m-%d"
      )
      ...
      DT::datatable(
        ...
        callback = JS(
          '$("button.buttons-csv").css("background","#0080b7"); 
                    return table;'
        ),
        extensions = 'Buttons',
        options = list(
